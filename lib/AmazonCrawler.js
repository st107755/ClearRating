...
const USER_DIRECTORY = './users.json';
...
module.exports = class AmazonCrawler {
    ...
    async init() {
        if (fs.existsSync(USER_DIRECTORY)) {
            try {
                this.users = JSON.parse(fs.readFileSync(USER_DIRECTORY, 'UTF-8'));
            } catch (e) {
     ...
     */
    async stop() {
        fs.writeFileSync(USER_DIRECTORY, JSON.stringify(this.users), 'UTF-8');
    ...
    async _crawlProductReviews(page, productId, pageIndex, pageLimit) {
        ...
        for (let i = 0; i < reviewElements.length; i++) {
            ...
            if (this.users[accountId]) {
                this.users[accountId]
            }
            let account = this.users[accountId] ? this.users[accountId] : await this.crawlProfile(accountId);
            account = Object.assign(account, {
                name: $('span.a-profile-name', element).text(),
                href: href,
                id: accountId
            });
            this.users[accountId] = account;
    ...
    async crawlProfile(accountId) {
        ...
        return {
            ...
            verifiedAverage: statistics.verifiedAverage(),
            totalDiviation: statistics.totalStandartDeviation(),
            verifiedDiviation: statistics.verifiedStandartDeviation()
