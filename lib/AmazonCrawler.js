...
module.exports = class AmazonCrawler {
    ...
    async crawlProductReviews(productId) {
        ...
        const reviews = await this._crawlProductReviews(page, productId, 1);
        await page.close();
        return reviews;
    }
    ...
    async crawlProfile(accountId) {
        ...
        await db.putProfile(profileData);
    ...
    async _scrapeReviewCard($, card, optinalFileds = false) {
        ...
        const stars = parseInt(/a-star-(\d)/.exec($('div:nth-child(2) > div.a-section.profile-at-content > a > div.a-section.a-spacing-mini > div > i', card).prop("class"))[1]);
