...
const {scrollToBottom, covertToDate, disableImages} = require('./Utils');
const fs = require('fs');
...
module.exports = class AmazonCrawler {
    ...
    async crawlProductReviews(productId) {
        ...
        product = await this._crawlProductReviews(page, productId, 1);
        await page.close();
    ...
    async _scrapeReviewCard($, card, optinalFileds = false) {
        //For Replace Operation (Change for different language)
